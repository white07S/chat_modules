...
export interface AgentConfig {
  ...
  mcpServers?: any;
  threadOpts: any; // ThreadOptions from Codex SDK
}
...
export class AgentManager {
  private agents: Map<string, { codex: Codex; config: AgentConfig }> = new Map();
  private threads: Map<string, ActiveThread> = new Map(); // threadId -> thread info
  private baseAgentPath: string;
  ...
  private async loadAgent(agentType: string) {
  ...
  startThread(agentType: string): { thread: Thread; threadId: string } {
  ...
  updateThreadActivity(threadId: string) {
  ...
  updateThreadId(oldThreadId: string | null, newThreadId: string) {
  ...
  async resumeThread(agentType: string, threadId: string): Promise<Thread> {
    const agent = this.getAgent(agentType);
  ...
  cleanupInactiveThreads(maxIdleMs: number = 30 * 60 * 1000) {
    const now = Date.now();
