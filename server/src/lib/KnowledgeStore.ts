...
const computeSqlHash = (sql: string): string => {
...
export const saveKnowledgeEntries = async (entries: KnowledgeEntryInput[]): Promise<KnowledgeSaveResult> => {
  if (entries.length === 0) {
  ...
  const dedupMap = new Map<string, typeof prepared[number]>();
