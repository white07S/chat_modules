...
class CodexChatServer {
        ...
        this.app.post('/knowledge', async (req, res) => {
            try {
                ...
                const normalizedQueries = queries
                    .map((sql) => (typeof sql === 'string' ? sql.trim() : ''))
                    .filter((sql) => sql.length > 0);
                if (normalizedQueries.length === 0) {
                    res.status(400).json({ error: 'At least one SQL query is required' });
                    return;
                }
                const { saved, duplicates } = await saveKnowledgeEntries(normalizedQueries.map((sql) => ({
    ...
    async persistUserMessageEvent(job, threadId, message) {
        ...
        await this.safeThreadOperation(() => this.threadStore.appendEvent(threadId, job.jobId, userEvent), {
